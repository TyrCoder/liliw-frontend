...
import { Loader2, Plus, Edit2, Trash2, Send, CheckCircle, AlertCircle, X, Palette } from 'lucide-react';
...
export default function ArtFormsTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const load = async (status?: string) => {
    ...
    const res = await fetch(`/api/cms/art-forms${qs}`, { headers: h }).catch(() => null);
  ...
  const save = async () => {
    ...
    const url = isNew ? '/api/cms/art-forms' : `/api/cms/art-forms/${editing.id}`;
  ...
  const submit = async (id: string) => {
    await fetch(`/api/cms/art-forms/${id}/submit`, { method: 'POST', headers: h });
    load(statusFilter);
  };

  const approve = async (id: string) => {
    await fetch(`/api/cms/art-forms/${id}/approve`, { method: 'POST', headers: h });
    load(statusFilter);
  };
  ...
  const confirmReject = async () => {
    ...
    setRejecting(true);
    await fetch(`/api/cms/art-forms/${rejectTarget}/reject`, { method: 'POST', headers: h, body: JSON.stringify({ remarks: rejectRemarks }) });
  ...
  const del = async (id: string) => {
    ...
    setDeleting(id);
    await fetch(`/api/cms/art-forms/${id}`, { method: 'DELETE', headers: h });
    setDeleting(null);
    load(statusFilter);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-base font-bold text-gray-900">Art Forms</h2>
          <p className="text-xs text-gray-400 mt-0.5">Traditional crafts and art forms of Liliw</p>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {entries.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-xl">{e.icon || '—'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 py-8 px-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">{editing.id ? 'Edit Art Form' : 'New Art Form'}</h3>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <RichTextEditor value={editing.features || ''} onChange={v => setEditing(p => ({ ...p, features: v }))} placeholder="List features and techniques…" />
              </div>
              <div>
