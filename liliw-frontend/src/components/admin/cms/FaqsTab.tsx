...
export default function FaqsTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const confirmReject = async () => {
    ...
    setRejecting(true);
    await fetch(`/api/cms/faqs/${rejectTarget}/reject`, { method: 'POST', headers: h, body: JSON.stringify({ remarks: rejectRemarks }) });
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <input value={editing.question || ''} onChange={e => setEditing(p => ({ ...p, question: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="What is…?" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Answer *</label>
                <textarea rows={5} value={editing.answer || ''} onChange={e => setEditing(p => ({ ...p, answer: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Category</label>
                  <input value={editing.category || ''} onChange={e => setEditing(p => ({ ...p, category: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="e.g. Travel, Permits" />
                </div>
                <div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <button onClick={save} disabled={saving || !editing.question?.trim() || !editing.answer?.trim()} className="inline-flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold text-white transition disabled:opacity-60" style={{ backgroundColor: '#1565C0' }}>
