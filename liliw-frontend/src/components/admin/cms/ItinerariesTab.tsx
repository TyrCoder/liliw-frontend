...
export default function ItinerariesTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const confirmReject = async () => {
    ...
    setRejecting(true);
    await fetch(`/api/cms/itineraries/${rejectTarget}/reject`, { method: 'POST', headers: h, body: JSON.stringify({ remarks: rejectRemarks }) });
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <input value={editing.title || ''} onChange={e => setEditing(p => ({ ...p, title: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="e.g. Liliw Day Tour" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Description</label>
                <textarea rows={3} value={editing.description || ''} onChange={e => setEditing(p => ({ ...p, description: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Category</label>
                  <input value={editing.category || ''} onChange={e => setEditing(p => ({ ...p, category: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="e.g. Nature, Culture" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Duration (days)</label>
                  <input type="number" min={1} value={editing.duration_days ?? 1} onChange={e => setEditing(p => ({ ...p, duration_days: Number(e.target.value) }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
              </div>
              <div>
