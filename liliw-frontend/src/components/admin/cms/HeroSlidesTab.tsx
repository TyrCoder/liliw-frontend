...
import { Loader2, Plus, Edit2, Trash2, Send, CheckCircle, AlertCircle, X, Image } from 'lucide-react';
...
export default function HeroSlidesTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const autoSaveStatus = useAutoSaveDraft(
    ...
    async () => {
      ...
      await fetch(`/api/cms/hero-slides/${editing.id}`, { method: 'PUT', headers: h, body: JSON.stringify({ ...editing, media, created_by: userEmail }) });
    }
  );
  ...
  const load = async (status?: string) => {
    ...
    const res = await fetch(`/api/cms/hero-slides${qs}`, { headers: h }).catch(() => null);
  ...
  const save = async () => {
    ...
    const url = isNew ? '/api/cms/hero-slides' : `/api/cms/hero-slides/${editing.id}`;
    const res = await fetch(url, { method: isNew ? 'POST' : 'PUT', headers: h, body: JSON.stringify(body) });
    const d = await res.json();
    if (res.ok) { setMsg({ ok: true, text: isNew ? 'Slide created.' : 'Slide saved.' }); load(statusFilter); setTimeout(closeForm, 1200); }
    else setMsg({ ok: false, text: d.error || 'Save failed' });
    setSaving(false);
  };

  const submit = async (id: string) => { await fetch(`/api/cms/hero-slides/${id}/submit`, { method: 'POST', headers: h }); load(statusFilter); };
  const approve = async (id: string) => { await fetch(`/api/cms/hero-slides/${id}/approve`, { method: 'POST', headers: h }); load(statusFilter); };
  const reject = (id: string) => { setRejectTarget(id); setRejectRemarks(''); };
  const confirmReject = async () => {
    if (!rejectTarget || !rejectRemarks.trim()) return;
    setRejecting(true);
    await fetch(`/api/cms/hero-slides/${rejectTarget}/reject`, { method: 'POST', headers: h, body: JSON.stringify({ remarks: rejectRemarks }) });
  ...
  const del = async (id: string) => {
    ...
    setDeleting(id);
    await fetch(`/api/cms/hero-slides/${id}`, { method: 'DELETE', headers: h });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-base font-bold text-gray-900">Hero Slides</h2>
          <p className="text-xs text-gray-400 mt-0.5">Homepage banner slides</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {['all','draft','pending','approved','rejected'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              ...
              style={statusFilter === s ? { backgroundColor: '#1565C0' } : {}}>{STATUS_LABELS[s]}</button>
          ))}
          {canEdit && (
            <button onClick={openCreate} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold text-white" style={{ backgroundColor: '#1565C0' }}>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 text-gray-400"><Image className="w-10 h-10 mb-3 opacity-20" /><p className="font-semibold text-sm">No slides yet</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wide">
                <th className="px-5 py-3 text-left">Title</th>
                <th className="px-5 py-3 text-left">Button</th>
                ...
                {entries.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-4">
                      ...
                      {e.subtitle && <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[200px]">{e.subtitle}</p>}
                      {e.reject_remarks && <p className="text-xs text-red-500 mt-0.5">Rejected: {e.reject_remarks}</p>}
                    </td>
                    <td className="px-5 py-4 text-gray-500 text-xs">{e.button_text || '—'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 py-8 px-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">{editing.id ? 'Edit Slide' : 'New Slide'}</h3>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Background Image</label>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <button onClick={save} disabled={saving || !editing.title?.trim()} className="inline-flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold text-white transition disabled:opacity-60" style={{ backgroundColor: '#1565C0' }}>
