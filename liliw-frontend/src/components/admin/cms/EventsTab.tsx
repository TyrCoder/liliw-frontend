...
import { Loader2, Plus, Edit2, Trash2, Send, CheckCircle, AlertCircle, X, Calendar } from 'lucide-react';
...
export default function EventsTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const load = async (status?: string) => {
    ...
    const res = await fetch(`/api/cms/events${qs}`, { headers: h }).catch(() => null);
  ...
  const save = async () => {
    ...
    const url = isNew ? '/api/cms/events' : `/api/cms/events/${editing.id}`;
  ...
  const submit = async (id: string) => {
    await fetch(`/api/cms/events/${id}/submit`, { method: 'POST', headers: h });
    load(statusFilter);
  };

  const approve = async (id: string) => {
    await fetch(`/api/cms/events/${id}/approve`, { method: 'POST', headers: h });
    load(statusFilter);
  };
  ...
  const confirmReject = async () => {
    ...
    setRejecting(true);
    await fetch(`/api/cms/events/${rejectTarget}/reject`, { method: 'POST', headers: h, body: JSON.stringify({ remarks: rejectRemarks }) });
  ...
  const del = async (id: string) => {
    ...
    setDeleting(id);
    await fetch(`/api/cms/events/${id}`, { method: 'DELETE', headers: h });
    setDeleting(null);
    load(statusFilter);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Festivals, seminars, workshops</p>
        </div>
                ...
                {entries.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-gray-400 text-xs">
                      {e.date_start ? new Date(e.date_start).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}
                    </td>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editing.category || 'cultural'} onChange={e => setEditing(p => ({ ...p, category: e.target.value }))}
                ...
                <div>
                  ...
                  <input type="datetime-local" value={editing.date_start?.slice(0,16) || ''} onChange={e => setEditing(p => ({ ...p, date_start: e.target.value }))}
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">End Date</label>
                  <input type="datetime-local" value={editing.date_end?.slice(0,16) || ''} onChange={e => setEditing(p => ({ ...p, date_end: e.target.value }))}
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <button onClick={save} disabled={saving || !editing.title?.trim()}
                  ...
                  style={{ backgroundColor: '#1565C0' }}>
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : null} Save Draft Draft
                </button>
              </div>
            </div>
          </div>
