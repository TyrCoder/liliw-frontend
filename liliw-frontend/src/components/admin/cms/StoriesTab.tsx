...
export default function StoriesTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const autoSaveStatus = useAutoSaveDraft(
    ...
    async () => {
      ...
      await fetch(`/api/cms/stories/${editing.id}`, { method: 'PUT', headers: h, body: JSON.stringify({ ...editing, media, created_by: userEmail }) });
    }
  );
  ...
  const confirmReject = async () => {
    ...
    setRejecting(true);
    await fetch(`/api/cms/stories/${rejectTarget}/reject`, { method: 'POST', headers: h, body: JSON.stringify({ remarks: rejectRemarks }) });
