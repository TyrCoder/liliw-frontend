...
import { Loader2, Plus, Edit2, Trash2, Send, CheckCircle, AlertCircle, X, Newspaper } from 'lucide-react';
...
export default function NewsTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const autoSaveStatus = useAutoSaveDraft(
    ...
    async () => {
      ...
      await fetch(`/api/cms/news/${editing.id}`, { method: 'PUT', headers: h, body: JSON.stringify({ ...editing, media, created_by: userEmail }) });
    }
  );
  ...
  const load = async (status?: string) => {
    ...
    const res = await fetch(`/api/cms/news${qs}`, { headers: h }).catch(() => null);
  ...
  const save = async () => {
    ...
    const url = isNew ? '/api/cms/news' : `/api/cms/news/${editing.id}`;
  ...
  const submit = async (id: string) => {
    await fetch(`/api/cms/news/${id}/submit`, { method: 'POST', headers: h });
    load(statusFilter);
  };

  const approve = async (id: string) => {
    await fetch(`/api/cms/news/${id}/approve`, { method: 'POST', headers: h });
    load(statusFilter);
  };
  ...
  const confirmReject = async () => {
    ...
    setRejecting(true);
    await fetch(`/api/cms/news/${rejectTarget}/reject`, { method: 'POST', headers: h, body: JSON.stringify({ remarks: rejectRemarks }) });
  ...
  const del = async (id: string) => {
    ...
    setDeleting(id);
    await fetch(`/api/cms/news/${id}`, { method: 'DELETE', headers: h });
    setDeleting(null);
    load(statusFilter);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-base font-bold text-gray-900">News</h2>
          <p className="text-xs text-gray-400 mt-0.5">Articles and announcements</p>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {entries.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-gray-500 capitalize">{e.category.replace('-', ' ')}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 py-8 px-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">{editing.id ? 'Edit News' : 'New News Article'}</h3>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <select value={editing.category || 'announcement'} onChange={e => setEditing(p => ({ ...p, category: e.target.value }))}
              ...
              <div>
                ...
                <RichTextEditor value={editing.content || ''} onChange={v => setEditing(p => ({ ...p, content: v }))} placeholder="Write the news article…" />
              </div>
              <div>
