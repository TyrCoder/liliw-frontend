...
import { Loader2, Plus, Edit2, Trash2, Send, CheckCircle, AlertCircle, X, MapPin } from 'lucide-react';
...
export default function AttractionsTab({ token, userEmail, isOfficer, isAdmin }: Props) {
  ...
  const autoSaveStatus = useAutoSaveDraft(
    ...
    async () => {
      ...
      await fetch(`/api/cms/attractions/${editing.id}`, { method: 'PUT', headers: h, body: JSON.stringify({ ...editing, media, created_by: userEmail }) });
    }
  );
  ...
  const load = async (status?: string) => {
    ...
    const res = await fetch(`/api/cms/attractions${qs}`, { headers: h }).catch(() => null);
  ...
  const save = async () => {
    ...
    const url = isNew ? '/api/cms/attractions' : `/api/cms/attractions/${editing.id}`;
  ...
  const submit = async (id: string) => {
    const res = await fetch(`/api/cms/attractions/${id}/submit`, { method: 'POST', headers: h });
    if (res.ok) load(statusFilter);
  };

  const approve = async (id: string) => {
    const res = await fetch(`/api/cms/attractions/${id}/approve`, { method: 'POST', headers: h });
    if (res.ok) load(statusFilter);
  };
  ...
  const del = async (id: string) => {
    ...
    setDeleting(id);
    await fetch(`/api/cms/attractions/${id}`, { method: 'DELETE', headers: h });
    setDeleting(null);
    load(statusFilter);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-base font-bold text-gray-900">Attractions</h2>
          <p className="text-xs text-gray-400 mt-0.5">Heritage sites, tourist spots, dining</p>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {entries.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-gray-500 capitalize">{e.category.replace('_', ' ')}</td>
                    <td className="px-5 py-4"><StatusBadge status={e.status} /></td>
                    <td className="px-5 py-4 text-gray-400 text-xs">{new Date(e.created_at).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 py-8 px-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">{editing.id ? 'Edit Attraction' : 'New Attraction'}</h3>
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Latitude</label>
                  <input type="number" value={editing.map_lat ?? ''} onChange={e => setEditing(p => ({ ...p, map_lat: e.target.value ? Number(e.target.value) : null }))}
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Longitude</label>
                  <input type="number" value={editing.map_lng ?? ''} onChange={e => setEditing(p => ({ ...p, map_lng: e.target.value ? Number(e.target.value) : null }))}
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300" />
                </div>
              </div>
              <div>
              ...
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Features</label>
                <RichTextEditor value={editing.features || ''} onChange={v => setEditing(p => ({ ...p, features: v }))} placeholder="List features, highlights, or key points…" />
              </div>
              <div>
